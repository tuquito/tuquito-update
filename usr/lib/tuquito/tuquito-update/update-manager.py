...
class RefreshThread(threading.Thread):
	...
	def run(self):
		...
		if self.glade.get_object('data').get_label() == '':
			self.glade.get_object('data').set_markup(_('Select a package to see its information'))
		gtk.gdk.threads_leave()
		if cant > 0:
			ready = True
			if self.synaptic:
...
class RefreshThread(threading.Thread):
	...
	def run(self):
		...
		self.glade.get_object('pkgSelected').set_markup(_('Packages selected: <b>%d</b>') % cant)
		self.glade.get_object('totalSize').set_markup(_('Download size: <b>%s</b>') % convert(totalSize))
		if self.glade.get_object('data').get_markup() == '':
			self.glade.get_object('data').set_label(_('Select a package'))
		gtk.gdk.threads_leave()
		if cant > 0:
			ready = True
			if self.synaptic:
